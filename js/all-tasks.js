...
const calculateTotalBalance = users => {
  const total = users.reduce((accumulator, user) => accumulator + user.balance, 0);
  return total;
};
...
const getNamesSortedByFriendsCount = users => 
 users.sort((next, prev) =>  next.friends.length - prev.friends.length).map(user => user.name);
...
const getSortedUniqueSkills = users => 
 users.flatMap(user => user.skills).filter((skill, index, arr) => arr.indexOf(skill) === index).sort();
